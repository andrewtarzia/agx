...
def test_building_block_configuration(graph_data: CaseData) -> None:
    ...
    iterator = agx.scram.TopologyIterator(
    ...
    if graph_data.num_configs == 0:
        with pytest.raises(RuntimeError) as e_info:
            agx.scram.get_custom_bb_configurations(iterator=iterator)
        ...
        possible_bbdicts = agx.scram.get_custom_bb_configurations(
            iterator=iterator
        )
        ...
        for bb_config in possible_bbdicts:
            ...
            for idx, topology_code in enumerate(iterator.yield_graphs()):
                if agx.scram.passes_graph_bb_iso(
