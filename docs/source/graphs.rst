...
You can use :meth:`get_configurations` to iterate over node configurations for
a given iterator, which can then be mapped to a graph using the containers
below.
...
use the `get_graph` method and/or the `is_isomorphic` method to check if two
graphs/configured graphs are the same.
